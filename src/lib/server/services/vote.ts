...
export async function castVote(userId: string, contestantId: string) {
  try {
    ...
    // Check if the contestant is enabled
    if (!contestant.enabled) {
      throw new Error(
        `Contestant with ID ${contestantId} is not enabled and cannot receive votes.`
      );
    }
    ...
    if (await hasUserVotedForContestant(userId, contestantId)) {
      throw new Error(
        `User with ID ${userId} has already voted for contestant ${contestantId}.`
      );
    }
...
export async function hasUserVotedForContestant(
  userId: string,
  contestantId: string
) {
